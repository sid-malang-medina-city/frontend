...
export default {
  name: 'manajemen-po-supplier-create',
  ...
  methods: {
    ...mapActions(POSupplierStore, ['createPOSupplier']),
    ...
    async submit () {
      ...
      try {
        await this.createPOSupplier(this.generatePayload())
        this.redirectTo('ManajemenPOSupplier')
        this.showToast('PO Supplier baru berhasil ditambahkan!')
      } catch (e) {
    ...
    calculateHargaTotalBarang () {
      this.form.harga_total = (parseFloat(this.form.harga.replace('.', '').replace(',', '.')) || 0) * (parseFloat(this.form.volume) || 0)
    }
  }
}
