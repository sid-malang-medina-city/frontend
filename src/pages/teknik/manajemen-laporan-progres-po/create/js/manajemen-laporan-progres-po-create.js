...
export default {
  name: 'manajemen-laporan-progres-po-create',
  ...
  methods: {
    ...mapActions(laporanProgresPOStore, ['createLaporanProgresPO']),
    ...
    async submit () {
      ...
      try {
        await this.createLaporanProgresPO(this.generatePayload())
        this.redirectTo('ManajemenLaporanProgresPO')
    ...
    generatePayload () {
      ...
      formData.progres_jenis_pekerjaans.forEach((jenisPekerjaan, index) => {
        ...
        if (!!jenisPekerjaan.harga_hari_lalu) {
          hargaProgresHariLalu += jenisPekerjaan.harga_hari_lalu
        }
        if (!!jenisPekerjaan.persentase_hari_lalu) {
          persentaseProgresHariLalu += jenisPekerjaan.persentase_hari_lalu
        }
        if (!!jenisPekerjaan.harga_hari_ini) {
          hargaProgresHariIni += jenisPekerjaan.harga_hari_ini
        }
        if (!!jenisPekerjaan.persentase_hari_ini) {
          persentaseProgresHariIni += jenisPekerjaan.persentase_hari_ini
        }
        if (!!jenisPekerjaan.harga_progres_total) {
          hargaProgresTotal += jenisPekerjaan.harga_progres_total
        }
        if (!!jenisPekerjaan.persentase_progres_total) {
          persentaseProgresTotal += jenisPekerjaan.persentase_progres_total
        }
      })
